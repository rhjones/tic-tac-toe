...
/***/ function(module, exports, __webpack_require__) {
	...
	var onSignUp = function onSignUp(event) {
	  ...
	  api.signUp(signUpData).done(function (data, textStatus, jqXHR) {
	    api.autoLogIn(data, textStatus, jqXHR, signUpData).done(logIn).fail(ui.logInFailure);
	...
	var onLogIn = function onLogIn(event) {
	  ...
	  api.logIn(data).done(logIn).fail(ui.logInFailure);
	};

	var onLogOut = function onLogOut(event) {
	  event.preventDefault();
	  api.logOut().done(ui.logOutSuccess).fail(ui.logOutFailure);
	};

	var onChangePassword = function onChangePassword(event) {
	  event.preventDefault();
	  var data = getFormFields(event.target);
	  api.changePassword(data).done(ui.passwordChangeSuccess).fail(ui.passwordChangeFailure);
	};

	var onToggleAuthOptions = function onToggleAuthOptions() {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var autoLogIn = function autoLogIn(userData, textStatus, jqXHR, signUpData) {
	  var request = $.ajax({
	    url: app.host + '/sign-in',
	    method: 'POST',
	    data: signUpData
...
/***/ function(module, exports) {
	...
	var app = {
	  host: 'https://tictactoe-rhj.herokuapp.com/',
	  alerts: {
	    welcome: {
	      class: 'welcome',
...
/***/ function(module, exports, __webpack_require__) {
	...
	var messageFade = function messageFade(location, messageKey) {
	  $(location).removeClass().addClass(app.alerts[messageKey].class).html(app.alerts[messageKey].msg).show().delay(3000).fadeToggle('slow');
	};

	var messageStick = function messageStick(location, messageKey) {
	  $(location).removeClass().addClass(app.alerts[messageKey].class).html(app.alerts[messageKey].msg).show();
	};

	var toggleAuth = function toggleAuth() {
	  $('.auth-nav').toggle();
	  $('.auth-forms').toggle();
	  $('.user-nav').toggle();
	  $('.game-play').toggle();
	...
	var toggleAuthOptions = function toggleAuthOptions() {
	...
	var toggleStats = function toggleStats() {
	  $('.stats').toggleClass('inline-block');
	};

	var clearCells = function clearCells() {
	  for (var i = 0; i < 9; i++) {
	    var cell = $('.game-board').find("[data-id='" + i + "']");
	    cell.html('');
	  }
	};

	var hideGameMessages = function hideGameMessages() {
	  $('.game-messages').hide();
	};

	var hideGameOver = function hideGameOver() {
	  $('#game-over').hide();
	};
	...
	var displayStats = function displayStats(stats) {
	...
	var endGameFailure = function endGameFailure() {
	  messageFade('.messages div', 'endGameFail');
	};

	var takeTurnFailure = function takeTurnFailure() {
	...
	var markCell = function markCell(id) {
	  var cell = $('.game-board').find("[data-id='" + id + "']");
	  if (game.xTurn) {
	...
	module.exports = {
	  toggleAuthOptions: toggleAuthOptions,
	  ...
	  endGame: endGame,
	  markCell: markCell,
	  indicatePlayer: indicatePlayer,
	  setPlayerX: setPlayerX,
	  hideGameMessages: hideGameMessages,
	  createGameFailure: createGameFailure,
	  endGameFailure: endGameFailure,
	  takeTurnFailure: takeTurnFailure,
...
/***/ function(module, exports, __webpack_require__) {
	...
	var getStats = function getStats(data) {
	  var stats = logic.calculateGameStats(data);
	  ui.displayStats(stats);
	};

	var onGetStats = function onGetStats() {
	  api.getFinishedGames().done(getStats).fail();
	};

	var onCreateGame = function onCreateGame() {
	  event.preventDefault();
	  api.createGame().done(logic.createGameSuccess).fail(ui.createGameFailure);
	};
	...
	var makeMove = function makeMove(data) {
	  var result = logic.takeTurnSuccess(data);

	  // if result is true or 'tie' (but not false)
	  if (result) {
	    api.endGame().done(onEndGameSuccess).fail(ui.endGameFailure);
	  } else {
	    ui.indicatePlayer();
	  }
	};

	var onClickCell = function onClickCell() {
	  event.preventDefault();
	  ...
	  if (logic.isValidMove(id)) {
	    api.takeTurn(id).done(makeMove).fail(ui.takeTurnFailure);
	  }
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	var takeTurn = function takeTurn(id) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var checkRows = function checkRows(cells, id) {
	...
	var checkCols = function checkCols(cells, id) {
	...
	var checkDiagonals = function checkDiagonals(cells, id) {
	  var win = false;
	...
	var checkWin = function checkWin(cells, id) {
	...
	var takeTurnSuccess = function takeTurnSuccess(data) {
	  game.currentGame = data.game;
	  game.currentGameMoves++;
	  var id = game.currentCellId;
	  ui.markCell(id);
	...
	var findWinner = function findWinner(game) {
	...
	var calculateGameStats = function calculateGameStats(data) {
	  var stats = {
	    wins: 0,
	    losses: 0,
	    ties: 0
	  };
	  var games = data.games; // games is an array
	...
	module.exports = {
	  ...
	  takeTurnSuccess: takeTurnSuccess,
...
/***/ function(module, exports, __webpack_require__) {
	...
	module.exports = __webpack_require__.p + "8683c5681dbb9577814076ca035862ac.png";

/***/ },
/* 21 */
/***/ function(module, exports, __webpack_require__) {

	module.exports = __webpack_require__.p + "1fad90dcd47b4276f192a234081c67e2.png";
